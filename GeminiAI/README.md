...
Welcome to the Gemini AI Chat Application!
This is a simple but practical desktop application that allows you to interact with Google's Gemini language models through a clean, graphical user interface (GUI).
...
- **Direct Gemini AI Integration**: Connects to the Gemini API and sends prompts directly to a configured Gemini model.
...
2. **A Gemini API Key**
   You need access to Google's Gemini API.

   General setup flow:
   - Create or sign in to the Google AI platform you use for Gemini API access.
...
- runtime dependencies such as `google.generativeai`, `tkhtmlview`, `Markdown`, and `python-dotenv`
- development tools such as `pytest`

#### Step 4: Configure Your API Key

The application reads the Gemini API key from an environment file named `.env`.
...
3. Configure the Gemini client and create a model instance.
...
- **`pyproject.toml`**: Defines the package metadata, required Python version, dependencies, optional dev dependencies, and the `gemini-chat` command.
- **`src/gemini_chat_app/chat_main.py`**: Contains the high-level flow of the application, including API setup, prompt loading, response handling, logging, and GUI startup.
- **`src/gemini_chat_app/chat_util_functions.py`**: Contains reusable helpers for prompt reading, interaction logging, clipboard handling, and response UI construction.
- **`tests/`**: Contains automated tests for both the main flow and the utility functions.

## How It Works: The Code Flow

When you run `gemini-chat`, the application follows this sequence:

1. The `main()` function in `chat_main.py` starts the program.
2. `load_dotenv()` loads environment variables from `.env`.
3. The code reads `GEMINI_API_KEY` and validates that it exists.
4. `google.generativeai.configure()` sets up the Gemini client.
5. A `GenerativeModel` instance is created with the configured model name and generation settings.
6. A chat session is started with `model.start_chat(...)`.
7. `get_prompt_from_md()` reads the content of `input_prompt.md`.
8. The prompt is sent with `chat_session.send_message(...)`.
9. The returned text is logged to `output_results.md` together with timestamps and total duration.
10. The response text is converted from Markdown to HTML.
11. `show_response_gui_window()` creates the main `tkinter` window.
12. `show_response()` builds the widgets that render the response and the action buttons.

## Notes About the Current Gemini Implementation

A few provider-specific details are worth knowing:

- The current model name in code is `gemini-2.0-flash-exp`.
- Generation settings are defined in `chat_main.py`.
...
- gRPC verbosity environment values are set in code to reduce noisy client output.

## Troubleshooting

If the application does not run as expected, check these common issues:

- **`Please set the GEMINI_API_KEY in the .env file`**
  The `.env` file is missing, empty, or does not contain `GEMINI_API_KEY`.
