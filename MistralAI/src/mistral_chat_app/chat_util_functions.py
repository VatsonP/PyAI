...
def show_response(parent, title_text, response_html, response_text):
    """Builds and displays the UI components for the model's response within a parent widget.
    This function populates a given tkinter parent widget (e.g., a root window or
    a frame) with the HTML-formatted response, a copy-to-clipboard button, and a
    close button. It does not create its own top-level window or run the mainloop.

    :param parent: The tkinter parent widget to populate.
    ...
    :returns: None
    """
    # Configure the parent's title. The caller is responsible for geometry, etc.
    parent.winfo_toplevel().title(title_text)

    response_frame = tk.Frame(parent)
    ...
    # Use winfo_toplevel() to ensure we get the root window for clipboard/destroy actions
    root_window = parent.winfo_toplevel()

    scroll_frame.bind(
        "<Control-c>", lambda event: root_window.clipboard_append(scroll_frame.get("1.0", "end-1c")))
    scroll_frame.bind(
        "<Control-v>", lambda event: scroll_frame.insert("insert", root_window.clipboard_get()))
    ...
    copy_button = tk.Button(
        ...
        command=lambda: copy_to_clipboard(root_window, response_text),
        font=("Arial", 12)
    )
